...
export async function convertBVHToVRMAnimation(
  ...
  options?: {
    scale?: number;
    /** 头颈双向 slerp；越小越稳。0 = 关闭平滑 */
    smoothAlpha?: number;
  }
): Promise<ArrayBuffer> {
  const scale = options?.scale ?? 0.01;
  const envSmooth =
    typeof process !== "undefined" && process.env?.VRMA_HEAD_SMOOTH_ALPHA != null
      ? Number(process.env.VRMA_HEAD_SMOOTH_ALPHA)
      : NaN;
  const smoothAlpha =
    options?.smoothAlpha ??
    (Number.isFinite(envSmooth) && envSmooth >= 0 ? envSmooth : 0.1);
  ...
  // ACCAD 等动捕：头/颈高频噪点 → 双向 slerp 平滑
  if (smoothAlpha > 0) {
    const names = collectHeadNeckBoneNames(vrmBoneMap, clip);
    smoothHeadNeckClipTracks(clip, names, smoothAlpha);
  }
